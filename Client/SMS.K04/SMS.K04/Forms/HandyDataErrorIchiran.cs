...
namespace SMS.K04.Forms
{
    ...
    /// <update></update>
    /// --------------------------------------------------
    public partial class HandyDataErrorIchiran : SystemBase.Forms.CustomOrderForm
    {
        #region 定数
        ...
        /// <update></update>
        /// --------------------------------------------------
        public HandyDataErrorIchiran(UserInfo userInfo, string menuCategoryID, string menuItemID, string title)
        ...
        protected override void InitializeLoadControl()
        {
            ...
            try
            {
                ...
                // シートのタイトルを設定
                shtMeisai.ColumnHeaders[0].Caption = Resources.HandyDataErrorIchiran_CaptureDate;
                shtMeisai.ColumnHeaders[1].Caption = Resources.HandyDataErrorIchiran_Work;
                shtMeisai.ColumnHeaders[2].Caption = Resources.HandyDataErrorIchiran_No;
                shtMeisai.ColumnHeaders[3].Caption = Resources.HandyDataErrorIchiran_ErrorNumber;
            }
            catch (Exception ex)
            {
        ...
        protected override bool RunSearchExec()
        {
            try
            {
                ...
                this.shtMeisai.Enabled = false;
                if (ComFunc.IsExistsTable(ds, Def_T_TEMPWORK.Name))
                {
                    this.shtMeisai.DataSource = ds.Tables[Def_T_TEMPWORK.Name];
                    // 最も左上に表示されているセルの設定
        ...
        protected override void fbrFunction_F08Button_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string tempID = ComFunc.GetFld(dr, Def_T_TEMPWORK.TEMP_ID);
                string torikomiFlag = ComFunc.GetFld(dr, Def_T_TEMPWORK.TORIKOMI_FLAG);
                SystemBase.Forms.CustomOrderForm frm = null;
                try
                {
                    if (torikomiFlag == TORIKOMI_FLAG.HIKIWATASHI_VALUE1)
                    {
                        // 引渡
                        frm = new HandyDataErrorMeisaiShuka(this.UserInfo, tempID);
                    }
                    else if (torikomiFlag == TORIKOMI_FLAG.SHUKA_VALUE1)
                    {
                        // 集荷
                        frm = new HandyDataErrorMeisaiShuka(this.UserInfo, tempID);
                    }
                    else if (torikomiFlag == TORIKOMI_FLAG.BOX_VALUE1)
                    {
                        // Box梱包
                        frm = new HandyDataErrorMeisaiBox(this.UserInfo, tempID);
                    }
                    else if (torikomiFlag == TORIKOMI_FLAG.KENPIN_VALUE1)
                    {
                        // 検品
                        frm = new HandyDataErrorMeisaiKenpin(this.UserInfo, tempID);
                    }
                    else if (torikomiFlag == TORIKOMI_FLAG.MEASURE_VALUE1)
                    {
                        // 計測
                        frm = new HandyDataErrorMeisaiMeasure(this.UserInfo, tempID);
                    }
                    else
                    {
                        // パレット梱包
                        frm = new HandyDataErrorMeisaiPallet(this.UserInfo, tempID);
                    }
                    frm.Icon = this.Icon;
                    frm.ShowDialog(this);
                }
