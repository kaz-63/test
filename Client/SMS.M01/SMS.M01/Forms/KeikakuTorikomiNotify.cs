...
namespace SMS.M01.Forms
{
    ...
    /// <update></update>
    /// --------------------------------------------------
    public partial class KeikakuTorikomiNotify : SystemBase.Forms.CustomOrderForm
    {
        #region フィールド
        ...
        /// <update></update>
        /// --------------------------------------------------
        public KeikakuTorikomiNotify(UserInfo userInfo, string menuCategoryID, string menuItemID, string title)
        ...
        /// </summary>
        /// <param name="userInfo">ログインユーザー情報</param>
        /// <param name="niukeName">荷受け名称</param>
        /// <create>Y.Gwon 2023/07/14</create>
        /// <update></update>
        /// --------------------------------------------------
        public KeikakuTorikomiNotify(UserInfo userInfo, string niukeName, string consignCd)
            : base(userInfo)
        {
            InitializeComponent();

            // 画面タイトル設定
            this.Title = ComDefine.TITLE_M0100200;

            this._niukeName = niukeName;
        ...
        protected override void DisplayClear()
        {
            ...
            try
            {
                this.txtNiukeName.Text = this._niukeName;
        ...
        protected override bool RunSearchExec()
        {
            try
            {
                ...
                var dt = conn.GetKeikakuTorikomiNotify(cond);
                ...
                this._mailHeaderId = UtilData.GetFld(dt, 0, Def_M_CONSIGN_MAIL.MAIL_HEADER_ID);
        ...
        protected override bool RunEditInsert()
        {
            try
            {
                ...
                // 荷受メールマスタ
                {
                    ...
                    dt.Columns.Add(Def_M_CONSIGN_MAIL.CONSIGN_CD);
                    dt.Columns.Add(Def_M_CONSIGN_MAIL.MAIL_HEADER_ID);

                    dt.Rows.Add(this._consignCD, this._mailHeaderId);
                    ds.Tables.Add(dt);
                }
                // 荷受メール明細マスタ
                {
                    var dt = new DataTable(Def_M_CONSIGN_MAIL_MEISAI.Name);
                    dt.Columns.Add(Def_M_CONSIGN_MAIL_MEISAI.MAIL_ADDRESS_FLAG);
                    dt.Columns.Add(Def_M_CONSIGN_MAIL_MEISAI.ORDER_NO);
                    dt.Columns.Add(Def_M_CONSIGN_MAIL_MEISAI.USER_ID);
                    dt.Columns.Add(Def_M_CONSIGN_MAIL_MEISAI.MAIL_HEADER_ID);

                    // TO, CC, BCCそれぞれの数ほどデータを追加
                    int idx = 0;
                ...
                string errMsgID;
                string[] args;
                if (!conn.SaveKeikakuTorikomiNotify(cond, ds, out errMsgID, out args))
                {
