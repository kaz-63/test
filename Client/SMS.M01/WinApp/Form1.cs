...
namespace WinApp
{
    public partial class Form1 : SystemBase.Forms.BaseLogin
    {
        ...
        protected override bool ExecuteLogin(string userID, string password, ref string errorMsgID)
        {
            ...
            this.UserInfo.SysInfo.MaxUserID = ComFunc.GetFldToInt32(dt, 0, Def_M_SYSTEM_PARAMETER.MAX_USER_ID);
            this.UserInfo.SysInfo.MaxPassword = ComFunc.GetFldToInt32(dt, 0, Def_M_SYSTEM_PARAMETER.MAX_PASSWORD);
            this.UserInfo.SysInfo.MinPassword = ComFunc.GetFldToInt32(dt, 0, Def_M_SYSTEM_PARAMETER.MIN_PASSWORD);
            this.UserInfo.SysInfo.MyPasswordChange = ComFunc.GetFld(dt, 0, Def_M_SYSTEM_PARAMETER.MY_PASSWORD_CHANGE);
            this.UserInfo.SysInfo.PasswordExpiration = ComFunc.GetFld(dt, 0, Def_M_SYSTEM_PARAMETER.PASSWORD_EXPIRATION);
            this.UserInfo.SysInfo.ExpirationFlag = ComFunc.GetFld(dt, 0, Def_M_SYSTEM_PARAMETER.EXPIRATION_FLAG);
            this.UserInfo.SysInfo.Expiration = ComFunc.GetFldToInt32(dt, 0, Def_M_SYSTEM_PARAMETER.EXPIRATION);
            this.UserInfo.SysInfo.ExpirationWarningFlag = ComFunc.GetFld(dt, 0, Def_M_SYSTEM_PARAMETER.EXPIRATION_WARNING_FLAG);
            this.UserInfo.SysInfo.ExpirationWarning = ComFunc.GetFldToInt32(dt, 0, Def_M_SYSTEM_PARAMETER.EXPIRATION_WARNING);
            this.UserInfo.SysInfo.DuplicationPastPassword = ComFunc.GetFld(dt, 0, Def_M_SYSTEM_PARAMETER.DUPLICATION_PAST_PASSWORD);
            this.UserInfo.SysInfo.PasswordCheck = ComFunc.GetFld(dt, 0, Def_M_SYSTEM_PARAMETER.PASSWORD_CHECK);
            // ログイン機能無効時のデフォルト設定取得
        ...
        private void InitializeExecuteClass()
        {
            ...
            string categoryID = "M01";
            string nameSpace = "SMS." + categoryID + ".Forms.";
            // M0100020 : 担当者マスタ
            {
                DataRow dr = dt.NewRow();
                dr["Title"] = "担当者Master";
                dr["ClassName"] = nameSpace + "UserMaster";
            ...
            // M0100040 : 納入先保守
            {
                DataRow dr = dt.NewRow();
                dr["Title"] = "納入先保守";
                dr["ClassName"] = nameSpace + "NonyusakiHoshu";
                dr["CategoryID"] = categoryID;
                dr["ItemID"] = "M0100040";
                dt.Rows.Add(dr);
            }
            // M0100050 : 梱包情報保守
            {
                DataRow dr = dt.NewRow();
                dr["Title"] = "梱包情報保守";
                dr["ClassName"] = nameSpace + "KonpoJohoHoshu";
                dr["CategoryID"] = categoryID;
                dr["ItemID"] = "M0100050";
                dt.Rows.Add(dr);
            }
            // M0100060 : 物件名保守
            {
                DataRow dr = dt.NewRow();
                dr["Title"] = "物件名マスタ保守";
                dr["ClassName"] = nameSpace + "BukkenMeiHoshu";
                dr["CategoryID"] = categoryID;
                dr["ItemID"] = "M0100060";
                dt.Rows.Add(dr);
            }
            // M0100070 : 名称マスタ保守
            {
                DataRow dr = dt.NewRow();
                dr["Title"] = "名称マスタ保守";
                dr["ClassName"] = nameSpace + "MeishoHoshu";
                dr["CategoryID"] = categoryID;
                dr["ItemID"] = "M0100070";
                dt.Rows.Add(dr);
            }
